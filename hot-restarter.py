...
def fork_and_exec():
    ...
    envoy_binary = sys.argv[1]      # The first argument is the Envoy binary path
    envoy_args = sys.argv[2:]       # The rest of the arguments are for Envoy itself
    logger.info("executing envoy with binary=%s args=%s", envoy_binary, envoy_args)

    child_pid = os.fork()
    if child_pid == 0:
        # Child process: execute Envoy with all provided args
        os.execv(envoy_binary, [envoy_binary] + envoy_args)
    else:
        # Parent process
